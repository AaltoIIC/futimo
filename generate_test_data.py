        ...
        for i in range(NUMBER_OF_ROWS):
            ...
            for i in range(NUMBER_OF_VARIABLES):
                if i < defined_variables: #if the distibution has been defined
                    ...
                    #If there were not enough variables defined in FILE_NAME_VARIABLE_DESCRIPTIONS, use gaussian function with mu = 0 and sigma = 1 for the rest of variables
                    data_row.append(random.gauss(0, 1))
            writer.writerow(data_row)
            start_date = start_date + datetime.timedelta(milliseconds=TIME_INTERVAL)

if __name__ == "__main__":
    variable_descriptions, variable_names = read_variable_descriptions()
    generate_data(variable_descriptions, variable_names)
